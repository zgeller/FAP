...
/**
 * Jeffreys (Jeffreys divergence, J-divergence) distance measure. Time series
 * must be the same length (n) and they should be non-negative:
 *
 * <blockquote> <img src="doc-files/JeffreysDistance-1.png"> </blockquote>
 * 
 * <ul>
 ...
 */
public class JeffreysDistance extends AbstractCopyableDistance {
    ...
    public double distance(TimeSeries series1, TimeSeries series2) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (y1 != y2) {
                if (y1 == 0)
                    distance += y2 * MathUtils.getZeroDenominator(); // -y2 * -MathUtils.getZeroDenominator();
                else if (y2 == 0)
                    distance += y1 * Math.log(y1 / MathUtils.getZeroDenominator());
                else
                    distance += (y1 - y2) * Math.log(y1 / y2);
            }
    ...
    public Object makeACopy(boolean deep) {
        JeffreysDistance copy = new JeffreysDistance();
        init(copy, deep);
        return copy;
    }
