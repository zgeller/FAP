...
/**
 * Taneja distance measure. Time series must be the same length (n) and they
 * should be non-negative:
 *
 * <blockquote> <img src="doc-files/TanejaDistance-1.png"> </blockquote>
 * 
 * <ul>
 *  <li> {@code 0*ln(0)} is treated as 0 (see [1]).
 ...
 */
public class TanejaDistance extends AbstractCopyableDistance {

    private static final long serialVersionUID = 1L;

    /**
     * Constructs a new Taneja distance measure.
     */
    public TanejaDistance() {
    }

    /**
     * Constructs a new Taneja distance measure, specifying whether calculated
     * distances should be stored in memory for reuse.
     * 
     * @param storing {@code true} if calculated distances should be stored in
     *                memory for reuse
     */
    public TanejaDistance(boolean storing) {
        super(storing);
    }
    ...
    public double distance(TimeSeries series1, TimeSeries series2) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (sum != 0) {
                ...
                if (y1 == 0)
                    distance += y2 * Math.log(y2 / MathUtils.getZeroDenominator());

                else if (y2 == 0)
                    distance += y1 * Math.log(y1 / MathUtils.getZeroDenominator());
                
                else
                    distance += sum * Math.log((sum / Math.sqrt(y1 * y2)) / 2);
    ...
    public Object makeACopy(boolean deep) {
        TanejaDistance copy = new TanejaDistance();
        init(copy, deep);
        return copy;
    }
