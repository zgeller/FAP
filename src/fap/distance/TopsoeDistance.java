...
/**
 * Topsøe distance measure. Time series must be the same length (n) and they
 ...
 */
public class TopsoeDistance extends AbstractCopyableDistance {

    private static final long serialVersionUID = 1L;

    /**
     * Constructs a new Topsøe distance measure.
     */
    public TopsoeDistance() {
    }
    ...
    public double distance(TimeSeries series1, TimeSeries series2) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (denominator == 0)
                denominator = MathUtils.getZeroDenominator();
            
            if (y1 != 0)
                distance += y1 * Math.log(2 * y1 / denominator);
            
            if (y2 != 0)
                distance += y2 * Math.log(2 * y2 / denominator);
    ...
    public Object makeACopy(boolean deep) {
        TopsoeDistance copy = new TopsoeDistance();
        init(copy, deep);
        return copy;
    }
