...
/**
 * Vicis symmetric chi-square 1 distance measure. Time series must be the same length
 * (n) and they should be non-negative:
 * 
 * <blockquote> <img src="doc-files/VicisSymmetricChiSquare1Distance-1.png"> </blockquote>
 * 
 * <ul>
 ...
 */
public class VicisSymmetricChiSquare1Distance extends AbstractCopyableDistance {

    private static final long serialVersionUID = 1L;

    /**
     * Constructs a new vicis symmetric chi-square 1 distance measure.
     */
    public VicisSymmetricChiSquare1Distance() {
    }

    /**
     * Constructs a new vicis symmetric chi-square 1 distance measure, specifying
     ...
     *                memory for reuse
     */
    public VicisSymmetricChiSquare1Distance(boolean storing) {
        super(storing);
    }
    ...
    public double distance(TimeSeries series1, TimeSeries series2) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (denominator != 0) {
                double tmp = (y1 - y2) / denominator;
                distance += tmp * tmp;
            }
            
            else if (y1 != y2) {
                double tmp = (y1 - y2) / MathUtils.getZeroDenominator();
                distance += tmp * tmp;
            }
    ...
    public Object makeACopy(boolean deep) {
        VicisSymmetricChiSquare1Distance copy = new VicisSymmetricChiSquare1Distance();
        init(copy, deep);
        return copy;
    }
